...
|---|---|---|---|
| `synapse-mcp` | `crates/synapse-mcp` | **54 MCP tools live** (M3 baseline plus #499 `act_keymap`, M4 `act_combo`/`act_run_shell`/`act_launch`, #508 `everquest_loc_probe`, #510 `everquest_current_state`, and M5 `profile_quality_refresh`, six `profile_authoring_*` tools, eight `profile_registry_*` tools including rollback, `audit_intelligence_query`, and #460 `audit_export_consent_set` / `audit_export_bundle`); streamable HTTP + SSE transport live on loopback with bearer auth + Origin/Host check + `Mcp-Session-Id` lifecycle; M3/M5 permission gating wired; profile, storage, reflex, authoring, registry, audit-intelligence, audit-export, and EverQuest current-state surfaces live | Continue M5 registry/audit moat work |
